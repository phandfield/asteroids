...
_state_log_initialized_cwd = False
_state_log_initialized_pkg = False
_start_time = datetime.now()


def log_state():
    global _frame_count, _state_log_initialized_cwd, _state_log_initialized_pkg
    ...
    cwd_path = Path.cwd() / "game_state.jsonl"
    pkg_path = Path(__file__).parent / "game_state.jsonl"

    mode_cwd = "w" if not _state_log_initialized_cwd else "a"
    with open(cwd_path, mode_cwd, encoding="utf-8") as f:
        f.write(json.dumps(entry) + "\n")

    mode_pkg = "w" if not _state_log_initialized_pkg else "a"
    with open(pkg_path, mode_pkg, encoding="utf-8") as f:
        f.write(json.dumps(entry) + "\n")

    _state_log_initialized_cwd = True
    _state_log_initialized_pkg = True
...
def log_event(event_type, **details):
    ...
    # Also write events to cwd and package dir
    cwd_evt = Path.cwd() / "game_events.jsonl"
    pkg_evt = Path(__file__).parent / "game_events.jsonl"

    mode_cwd = "w" if not _event_log_initialized else "a"
    with open(cwd_evt, mode_cwd, encoding="utf-8") as f:
        f.write(json.dumps(event) + "\n")

    mode_pkg = "w" if not _event_log_initialized else "a"
    with open(pkg_evt, mode_pkg, encoding="utf-8") as f:
        f.write(json.dumps(event) + "\n")
